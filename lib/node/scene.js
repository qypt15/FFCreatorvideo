...
class FFScene extends FFCon {
  ...
  addBlankImage() {
    let blankPng = this.conf.blankPng
  	if(blankPng != undefined && blankPng != null && blankPng != ''){
  		const blank = path.join(__dirname, blankPng);
  ...
  createFilepath() {
    ...
    // cacheDir  = /cache/
    // pathId  = o0hb68rg9gx5me85
    // id  = scene1
    // format  = mp4
    // cache\/o0hb68rg9gx5me85/scene1.mp4
    const filepath = `${directory}/${id}.${format}`;
    fs.ensureDir(dir);
    // console.log('镜头输出路径+文件名称-filepath',filepath)
  ...
  toCommand() {
    ...
    this.addNodesInputCommand(command); // 子节点 输入文件
    this.toFiltersCommand(command);    // 过滤器
    this.addCommandOptions(command);   // 其他项
    this.addCommandOutputs(command);    // 添加自定义输出选项 ,创建输出文件夹
    this.addNodesOutputCommand(command); // 子节点 输出
  ...
  deleteCacheFile() {
    ...
    if(debug) console.log('删除镜头缓存-debug',debug)
  ...
  isReady() {
    const cachePath = this.createCachePath() // 先创建缓存目录 
    return new Promise(resolve => {
      let readyIndex = 0;
      forEach(this.children, child => { 
        child.isReady(cachePath).then(() => {
          readyIndex++;
