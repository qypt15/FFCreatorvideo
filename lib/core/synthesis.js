...
class Synthesis extends FFBase {
  ...
  async start() {
    ...
    if (upStreaming) {
      ...
      if (ScenesUtil.hasTransition(creator)) {
        // 有转场，使用分散模式 (-i a.mp4 -i b.mp4)
        ScenesUtil.fillTransition(creator);
        this.addXfadeInput();
      } else {
        // 没有转场，就用 txt 集合模式 (-f concat -safe 0 -i a.txt)
  ...
  addXfadeInput() {
    ...
    console.log('complexFilter-cid',cid)
    console.log('complexFilter-filters',filters)
    if (isEmpty(cid) || isEmpty(filters)) return;
     console.log('有complexFilter-filters')
  ...
  addOutputOptions() {
    ...
    if (!isEmpty(audio)) {
      ...
      // 多视频，就不对了
      // command.outputOptions(['-filter_complex', `[0:a]volume=${vVolume}[a0];[1:a]volume=${bgmVolume}[a1];[a0][a1]amix=inputs=2[a]`]);
      // command.outputOptions(['-map', '0:v']);
      // command.outputOptions(['-map', '[a]']);
      let apro = '';
      const audioVolume = conf.getVal('audioVolume');
      const videoVolume = conf.getVal('videoVolume');

     // console.log('audioVolume',audioVolume) // 背景音乐 音量
     // console.log('videoVolume',videoVolume) // 原视频 音量
      if (ScenesUtil.hasTransition(creator)) {
        // 有转场 （-i a.mp4模式）
        ...
        if (audioVolume !== undefined) {
          // 调节音量 -af "volume=0.1"
          this.command.outputOptions(['-af', `volume=${audioVolume}`]);
        }
      }else{
        // 没有转场  (txt模式)
        if (audioVolume !== undefined) {
          // console.log('调节音量',videoVolume)
          // 调节音量
          this.command.outputOptions(['-filter_complex', `[0:a]volume=${videoVolume}[a0];[1:a]volume=${audioVolume}[a1];[a0][a1]amix`]);
        }else{
          // 不调节音量
          // console.log('不调节音量')
          this.command.outputOptions(['-filter_complex', '[0:a][1:a]amix']);
        }
