...
/**
 * 渲染器 Renderer - Core classes for rendering animations and videos.
 *
 * ####Example:
 *
...
class Renderer extends FFBase {
  ...
  async singleStart(scene) {
    if (!scene) return;
    // 转换视频
    // scene.on('videoa-progress', this.eventHandler.bind(this));
    const { creator } = this;
    scene.on('model-progress', this.eventHandler.bind(this));
    await scene.isReady();
  ...
  eventHandler(event) {
    ...
    switch (event.type) {
      ...
      case 'model-progress':
          ...
          // .event.model_key},${e.model_name}, ${(e.percent
          event.type = 'model-progress';
          // event.model_key = 'model-progress';
          // event.model_name = 'model-progress';
          event.model_key = event.model_key || 'model_key';
          event.model_name = event.model_name || 'model_name';
  ...
  async synthesisOutput() {
    ...
    synthesis.on('synthesis-progress', event => {
      event.type = 'progress';
      event.percent = this.percent + event.percent * 0.3;  // 进度
      this.emits(event);
    });
