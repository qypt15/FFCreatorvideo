...
class TaskQueue {
  ...
  push(task) {
    ...
    // console.log('dev-this.queue',this.queue)
    return id;
  }
  ...
  getListener(id, name) {
    const taskObj = this.getTaskById(id);
    console.log('getListener-taskObj',taskObj)
    if (taskObj) {
...
const FFCreatorCenter = {
   ...
   */
  onTaskError(id, func) {
    console.log('Listen to production task Error events',id)
  ...
  async execTask(taskObj) {
    ...
    try {
      ...
      console.log('taskObj','execTask: await taskObj.task(taskObj.id) return nul')
      if (!creator) {
        this.handlingError({
          taskObj,
  ...
  handlingError({ taskObj, error = 'normal' }) {
    ...
    this.event.emit(eventName, result);
    console.log('handlingError-eventName',eventName)
    console.log('handlingError-this.nextTask.bind(this)',this.nextTask.bind(this))
    setTimeout(id => this.removeTaskObj(id), 50, id);
    setTimeout(this.nextTask.bind(this), this.delay);
  },
